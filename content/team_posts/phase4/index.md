...
### App Description
Every year, students are tasked with picking the right university for them, without knowing how the job market will look when they graduate. We have built an application that connects EU university data with a decade of Eurostat labor statistics to help students, labor statisticians, and budget planners make the right decisions.

### User Personas

#### Students
Students are able to fill out a survey about their preferences in regards to choosing a university, and are given a personalized list of universities with coresponding match scores generated by our cosine similarity ML model. Students are then able to save their favorite universities, and write pros and cons lists for each universities to track their decision making process.

#### Labor Statisticians
Labor Statisticians are able to see predictions about the future of employment by sector within the 27 different EU countries. Based on almost a decade of data, our linear regression ML model predicts changes in employment and models the absorbtion of graduates into their respective sectors. Labor Statisticians are also able to view a variety of interactive charts to back up our model's predictions.

#### Budget Managers
Budget Managers are able to view a variety of statistics about each university, including their total budget, number of students, and graduation rate. Beyond these general statistics, Budget Managers can see how many students have favorited each university, offering them insights into the popularity of universities among incoming students. From here, Budget Managers can model different budgets and predict best allocation fit to the university's departments based on their given sector's needs in the university's country. These sector's needs are predicted using an OLS linear regression model trained on data from the Labor Statistician portal. Budget Managers can then save university budget plans for later reference.
